...
def rc_approximation(X, W_est, C_true, semiring,  epsilon=0.1,):
    '''
    Adapted from SparceRC repo
    ...
    if not non_zero_true is 0 and not C_true is 0:
        visualize_rc(non_zero_true[:25], non_zero_est[:25], semiring)
...
def visualize_rc(non_zero_true, non_zero_est, semiring):
    '''
    Adapted from SparceRC repo
    '''
    ...
    plt.savefig('neurips_experiments/plots/root_causes_{}.pdf'.format(semiring), dpi=1000)
